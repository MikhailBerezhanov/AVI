...
struct MyTcpHandler::Impl
{
	...
	int cb_period = 0;
	int cb_timer = 0;
	std::function<void()> periodic_callback = nullptr;
...
//
void MyTcpHandler::add_periodic_callback(int period_sec, std::function<void()> &&callback)
{
	pimpl->cb_period = period_sec;
	pimpl->periodic_callback = std::move(callback);
}
...
void MyTcpHandler::process_periodic_callback()
{
	if( !pimpl->periodic_callback ){
		return;
	}

	++pimpl->cb_timer;

	if(pimpl->cb_timer >= pimpl->cb_period){
		pimpl->cb_timer = 0;

		try{
			pimpl->periodic_callback();
		}
		catch(const std::exception &e){
			logging_excp(logger, std::string("periodic_callback throws: ") + e.what());
		}
	}
}
	...
	for(;;){
		...
		if(res < 0){
			...
			if( !FD_ISSET(pimpl->fd, &pimpl->readfds) && !FD_ISSET(pimpl->fd, &pimpl->writefds) ){
				...
				this->process_periodic_callback();
			}
