...
int main(int argc, char* argv[])
{
	...
	for(;;){
		...
		channel_ptr->onReady([&]()
		{
			...
			channel_ptr->declareQueue("dev_status");
		});


		// Add periodic producing
		myHandler.add_periodic_callback(10, [&](){
			std::string payload(250, '0');
			if(channel_ptr->publish("", "dev_status", payload)){
				logger.msg(MSG_DEBUG, "[x] Message (size: %zu) sent to 'dev_status' queue\n", payload.size());
			}
